//
// $Id: zapit.cpp 15062026 mohousch Exp $
//
...
void CZapit::initFrontend()
{
	...
	fe = new CFrontend(0, 4); // adapter_num = 4
...
int CZapit::zapToRecordID(const t_channel_id channel_id)
{
	...
	CFrontend * fe = getFrontend(rec_channel);
...
void CZapit::getCurrentPIDS( PIDs &pids )
{
	if (live_channel) 
	{
		OtherPIDs responseGetOtherPIDs;
		APIDList apids;
		SubPIDList subpids;
...
//
void CZapit::getPIDS( t_channel_id chid, PIDs &pids )
{
	CZapitChannel * channel = findChannelByChannelID(chid);
...
void CZapit::getRecordPIDS(PIDs &pids)
{
	//FIXME:
	if (rec_channel) 
	{
...
void CZapit::setScanSatelliteList( ScanSatelliteList &satelliteList )
{
	...
	{
		scanSatelliteList_t sat = satelliteList[i];
...
//
bool CZapit::scanTP(scanTP_t &msg)
{
...
//
bool CZapit::startScan(scanProvider_t &msg)
{		
...
void * CZapit::scanThread(void * data)
{
	...
	CZapit::scanProvider_t params = *(CZapit::scanProvider_t *)data;
...
void * CZapit::scanTransponderThread(void *data)
{
	...
	CZapit::scanTP_t params = *(CZapit::scanTP_t *)data;
...
void CZapit::parseChannels(xmlNodePtr node, const t_transport_stream_id transport_stream_id, const t_original_network_id original_network_id, t_satellite_position satellitePosition, freq_id_t freq)
{
	...
	while ((node = xmlGetNextOccurence(node, "S")) != NULL) 
	{
		*have_ptr = 1;
		...
		if(ret.second == false) 
		{
			dprintf(DEBUG_NORMAL, "CZapit::parseChannels: duplicate channel %s id 0x%llx freq %d (old %s at %d)\n", name.c_str(), chid, freq, ret.first->second.getName().c_str(), ret.first->second.getFreqId());
		} 
		else 
		{
			scnt++;
...
{
	dprintf(DEBUG_INFO, "CZapit::makeBouquetfromCurrentservices\n");
...
void CZapit::parseWebTVBouquet(std::string &filename)
{
	...
	if( strcasecmp("xml", extension.c_str()) == 0)
	{
		...
		if (parser) 
		{
			...
			if (l1) 
			{
				while ( ( (xmlGetNextOccurence(l1, "webtv")) || (xmlGetNextOccurence(l1, "webradio")) ) ) 
				{
					t_channel_id id = 0;
					...
					const char * logo;
					const char * epgid;
					bool isWebRadio = xmlGetNextOccurence(l1, "webradio");
					
					// 
					if( xmlGetNextOccurence(l1, "webtv") || xmlGetNextOccurence(l1, "webradio") )
					{
						...
						allchans.insert(std::pair<t_channel_id, CZapitChannel> (id, CZapitChannel(title, id, url, description, isWebRadio? ST_DIGITAL_RADIO_SOUND_SERVICE : ST_DIGITAL_TELEVISION_SERVICE)));
...
void CZapit::makeRemainingChannelsBouquets(void)
{
	...
	//
	remainChannels = addBouquet((Bouquets.size() == 0) ? _("All Channels") : _("Other"));
...
void CZapit::addChannelToBouquet(const unsigned int bouquet, const t_channel_id channel_id)
{
	...
	// skip webtv
	if (IS_WEBTV(channel_id) || Bouquets[bouquet]->bWebTV)
		return;
