...
//// scanManager
bool CZapit::tuneFrequency(FrontendParameters *feparams, t_satellite_position satellitePosition, int feindex)
{
...
bool CZapit::getSDTS(t_satellite_position satellitePosition, int feindex)
{
...
bool CZapit::scanTransponder(xmlNodePtr transponder, uint8_t diseqc_pos, t_satellite_position satellitePosition, int feindex)
{
...
bool CZapit::scanProvider(xmlNodePtr search, t_satellite_position satellitePosition, uint8_t diseqc_pos, bool satfeed, int feindex)
{
	...
	if (scanInputParser)
	{
		...
		while ( (search = xmlGetNextOccurence(search, frontendType)) != NULL ) 
		{
			...
			if (spI != scanProviders.end()) 
			{
				...
				if ( !CZapit::getInstance()->scanProvider(search, position, diseqc_pos, satfeed, feindex) )
				{
					found_channels = 0;
					break;
				}
...
void * CZapit::scanTransponderThread(void * data)
{
	...
	CZapit::getInstance()->addToScan(CREATE_TRANSPONDER_ID(freq, satellitePosition, CZapit::getInstance()->fake_nid, CZapit::getInstance()->fake_tid), &TP->feparams, false, feindex);

	// scanSDTS
	if (!CZapit::getInstance()->getSDTS(satellitePosition, feindex))
		found_channels = 0;
...
void CZapit::parseTransponders(xmlNodePtr node, t_satellite_position satellitePosition, fe_type_t frontendType)
{
	...
	while ((node = xmlGetNextOccurence(node, "TS")) != NULL) 
	{
		...
		if (ret.second == false)
			printf("CZapit::parseTransponders: duplicate transponder id %llx freq %d\n", tid, feparams.frequency);
