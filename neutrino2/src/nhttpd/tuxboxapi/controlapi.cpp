...
void CControlAPI::SetModeCGI(CyhookHandler *hh)
{
	if (!(hh->ParamList.empty()))
	{
		...
		if (hh->ParamList["1"] == "radio")	// switch to radio mode
		{
			...
			eventServer->sendEvent(NeutrinoMessages::CHANGEMODE, (void *)&mode, sizeof(int));
			sleep(1);
		}
...
void CControlAPI::StandbyCGI(CyhookHandler *hh)
{
	if (!(hh->ParamList.empty()))
	{
		...
		{
			eventServer->sendEvent(NeutrinoMessages::STANDBY_ON);
			hh->SendOk();
		}
		else if (hh->ParamList["1"] == "off")// standby mode off
		{
			eventServer->sendEvent(NeutrinoMessages::STANDBY_OFF);
			hh->SendOk();
		}
		else
			hh->SendError();
...
void CControlAPI::RCCGI(CyhookHandler *hh)
{
	if (!(hh->ParamList.empty()))
	{
		if (hh->ParamList["1"] == "lock")	// lock remote control
			eventServer->sendEvent(NeutrinoMessages::LOCK_RC);
...
void CControlAPI::MessageCGI(CyhookHandler *hh)
{
	...
	if (event != 0)
	{
		...
		eventServer->sendEvent(event, (void *) message.c_str(), message.length() + 1);
		hh->SendOk();
	}
	else
		hh->SendError();
...
void CControlAPI::RebootCGI(CyhookHandler *hh)
{
	if (hh->ParamList.empty())
	{
		eventServer->sendEvent(NeutrinoMessages::REBOOT);
		hh->SendOk();
	}
	else
		hh->SendError();
...
void CControlAPI::RestartCGI(CyhookHandler *hh)
{
	if (hh->ParamList.empty())
	{
		eventServer->sendEvent(NeutrinoMessages::RESTART);
		hh->SendOk();
	}
	else
		hh->SendError();
...
//				hh->printf("<bouquet>\n\t<bnumber>%s</bnumber>\n</bouquet>\n",hh->ParamList["bouquet"].c_str());
			...
			else 
			{
				...
				for (int i = 0; i < (int) CZapit::getInstance()->Bouquets.size(); i++) 
				{
					channels = mode == CZapit::MODE_RADIO ? CZapit::getInstance()->Bouquets[i]->radioChannels : CZapit::getInstance()->Bouquets[i]->tvChannels;
					int num = 1 + (mode == CZapit::MODE_RADIO ? CZapit::getInstance()->radioChannelsBegin().getNrofFirstChannelofBouquet(i) : CZapit::getInstance()->tvChannelsBegin().getNrofFirstChannelofBouquet(i));
...
void CControlAPI::GetBouquetsCGI(CyhookHandler *hh)
{
	...
	for (int i = 0; i < (int) CZapit::getInstance()->Bouquets.size(); i++) 
	{
		...
		if(!channels->empty() && (!CZapit::getInstance()->Bouquets[i]->bHidden || show_hidden))
		{
			bouquet = std::string(CZapit::getInstance()->Bouquets[i]->bFav ? _("Favorites") :CZapit::getInstance()->Bouquets[i]->Name.c_str());
			if(encode)
...
void CControlAPI::EpgCGI(CyhookHandler *hh)
{
	...
	else if (!(hh->ParamList["xml"].empty()))
	{
		...
		if(bouquetnr>=0)
		{
			...
			channels = mode == CZapit::MODE_RADIO ? CZapit::getInstance()->Bouquets[bouquetnr]->radioChannels : CZapit::getInstance()->Bouquets[bouquetnr]->tvChannels;
			std::string bouquet = std::string(CZapit::getInstance()->Bouquets[bouquetnr]->bFav ? _("Favorites") :CZapit::getInstance()->Bouquets[bouquetnr]->Name.c_str());
			bouquet = encodeString(bouquet); // encode (URLencode) the bouquetname
		...
		else if(all_bouquets)
		{
			...
			{
				channels = mode == CZapit::MODE_RADIO ? CZapit::getInstance()->Bouquets[i]->radioChannels : CZapit::getInstance()->Bouquets[i]->tvChannels;
				std::string bouquet = std::string(CZapit::getInstance()->Bouquets[i]->bFav ? _("Favorites") :CZapit::getInstance()->Bouquets[i]->Name.c_str());
				bouquet = encodeString(bouquet); // encode (URLencode) the bouquetname
				hh->printf("\t<bouquet>\n\t\t<number>%d</number>\n\t\t<name><![CDATA[%s]]></name>\n\t", i+1, bouquet.c_str());
...
void CControlAPI::StartPluginCGI(CyhookHandler *hh)
{
	...
	if (!(hh->ParamList.empty()))
	{
		if (hh->ParamList["name"] != "")
		{
			...
			eventServer->sendEvent(NeutrinoMessages::EVT_START_PLUGIN, (void *) pluginname.c_str(), pluginname.length() + 1);
