...
{
	dprintf(DEBUG_INFO, ANSI_GREEN"CRCInput::postMsg: msg:(0x%x) data:(0x%x) prio:(%d) >\n", msg, data, Priority );
...
////
void CRCInput::sendEvent(const neutrino_msg_t msg, void *data, const unsigned int size, const bool Priority)
{
	struct event buf;
	...
	//if ( p != NULL )
	{
		switch (msg)
		{
			...
			case NeutrinoMessages::EVT_ZAP_COMPLETE:
			{
				t_channel_id chan = (t_channel_id) data;
