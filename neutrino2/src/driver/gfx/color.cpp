...
#ifdef COLOR_DEBUG
#define color_printf(level, fmt, x...) do { \
if (debug_level >= level) printf("[%s:%s] " fmt, __FILE__, __FUNCTION__, ## x); } while (0)
#else
#define color_printf(level, fmt, x...)
#endif

#ifndef COLOR_SILENT
#define color_err(fmt, x...) do { printf("[%s:%s] " fmt, __FILE__, __FUNCTION__, ## x); } while (0)
#else
#define color_err(fmt, x...)
#endif
...
int blit(gUnmanagedSurface *m_surface, const int src_w, const int src_h, const CBox &_pos, gUnmanagedSurface * surface, int flag)
{
	CBox pos = _pos;

    	color_printf("source size: %d %d", pos.iWidth, pos.iHeight);
...
//        	color_printf("srcarea after scale: %d %d %d %d\n", srcarea.x(), srcarea.y(), srcarea.width(), srcarea.height());
        	...
        	if (flag & blitScale)
        	{
            		...
            		{
                		color_printf("unimplemented: scale on non-accel surface %d->%d bpp\n", m_surface->bpp, surface->bpp);
            		}
            		continue;
        	}
        	...
        	{
            		color_printf("cannot blit %dbpp from %dbpp\n", surface->bpp, m_surface->bpp);
            		return -1;
        	}
    	}
