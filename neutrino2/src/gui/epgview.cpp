...
void CEpgData::GetEPGData(const t_channel_id channel_id, uint64_t id, time_t *starttime, bool clear)
{
	...
	if ( id != 0 )
		...
		res = CSectionsd::getInstance()->getActualEPGServiceKey(CZapit::getInstance()->getChannelEPGID(channel_id) & 0xFFFFFFFFFFFFULL, &epgData);
...
////
int CEPGDataHandler::exec(CTarget *parent, const std::string &/*actionKey*/)
{
