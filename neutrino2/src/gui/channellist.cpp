...
//				paint(customMode);
		...
		else if (msg == CRCInput::RC_epg) // eventlist
		{
			...
			if ( chanlist.size() && g_EventList->show(chanlist[selected]->epgid, chanlist[selected]->name) == CTarget::RETURN_EXIT_ALL) 
			{
				res = -2;
				loop = false;
			}
...
int CChannelList::numericZap(int key)
{
	...
	{
		t_channel_id channel_id = CNeutrinoApp::getInstance()->getlastChList().getlast(1);
		if(channel_id) 
		{
			CNeutrinoApp::getInstance()->getlastChList().clear_storedelay(); // ignore store delay
	...
	if (key == g_settings.key_zaphistory) 
	{
		...
		// zap history bouquet
		if (CNeutrinoApp::getInstance()->getlastChList().size() > 1) 
		{
			CChannelList * channelList = new CChannelList(_("History"), true);

			for(unsigned int i = 1; i < CNeutrinoApp::getInstance()->getlastChList().size(); ++i) 
			{
				t_channel_id channel_id = CNeutrinoApp::getInstance()->getlastChList().getlast(i);
...
void CChannelList::paintCurrentNextEvent(int _selected)
{
	...
	nextEventslistBox.setInFocus(false);
	...
	{		
		...
		switch(select) 
		{
			...
			case 1: // move
				...
				do {
					new_bouquet_id = bouquetList->exec(false, false, true);
			...
			case 2: // add to
				...
				do {
					bouquet_id = bouquetList->exec(false, false, true);
				} while(bouquet_id == -3);
