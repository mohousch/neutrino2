...
void CInfoViewer::showTitle(const int _ChanNum, const std::string& _ChannelName, const t_satellite_position _satellitePosition, const t_channel_id _new_channel_id, const bool _calledFromNumZap, int _epgpos)
{
	...
	if ( (_satellitePosition != 0 && satellitePositions.size()) || (IS_WEBTV(channel_id))) 
	{
		...
		{
			// ChannelName
			g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_CHANNAME]->RenderString( BoxStartX + CHANNUMBER_WIDTH + BORDER_LEFT, ChanNameY + ChanNameHeight, ChanNameWidth, ChannelName, COL_INFOBAR_TEXT_PLUS_0);	// UTF-8
		}
	}
...
void CInfoViewer::showButton_Audio()
{
	...
	if(is_visible)
	{
	  	...
	  	if (count > 0 || (IS_WEBTV(channel_id))) 
		{
			...
			g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL]->RenderString(sx, buttonBarStartY + (buttonBarHeight - g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL]->getHeight())/2 + g_Font[SNeutrinoSettings::FONT_TYPE_INFOBAR_SMALL]->getHeight(), asize - 5 - icon_w, _("Audio"), COL_MENUFOOT_TEXT_PLUS_0, 0, true); // UTF-8
	  	}
	}
