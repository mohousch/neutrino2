...
class CPicViewer : public CTarget
{
	private:
...
void CPicViewer::showMenu()
{
	...
	for(unsigned int i = 0; i < (unsigned int)playlist.size(); i++)
	{
		...
		item = new CMenuForwarder(tmp.c_str(), true, NULL, this, "view");
	...
	plist->setMode(ClistBox::MODE_LISTBOX);
	plist->setLayout(ClistBox::LAYOUT_FRAME);
...
int CPicViewer::exec(CTarget* parent, const std::string& actionKey)
{
