...
--[[
	...
	menu:setHeadGradient(neutrino2.LIGHT2DARK)
	menu:setHeadLine(false)
	menu:setFootCorner(neutrino2.RADIUS_SMALL)
	menu:setFootGradient(neutrino2.DARK2LIGHT)
...
function mtv_liste(id)
	...
	menu:setLayout(neutrino2.ClistBox_LAYOUT_FRAME)
	...
	if glob.MTVliste ~= nil then
		for i, v in ipairs(glob.MTVliste) do
			item = neutrino2.CMenuForwarder(v.name, true, "", null, "play")
...
function settings()
	...
	local item = nil
	menu:setMode(neutrino2.MODE_SETUP)
...
function searchliste(id)
	...
	for i, v in ipairs(glob.MTVliste) do
		item = neutrino2.CMenuForwarder(v.name)
...
function main_menu()
	...
	menu:setMode(neutrino2.MODE_MENU)
	menu:enableShrinkMenu()
	menu:enablePaintDate()
	menu:setHeadCorner(neutrino2.RADIUS_SMALL)
	menu:setHeadGradient(neutrino2.LIGHT2DARK)
