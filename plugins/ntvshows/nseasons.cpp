...
void CNSeasons::showMenu()
{
	...
	for (unsigned int i = 0; i < m_vMovieInfo.size(); i++)
	{
		...
		item = new ClistBoxItem(m_vMovieInfo[i].epgTitle.c_str(), true, NULL, new CNEpisodes(season_id, i), NULL, RC_nokey, NULL, file_exists(m_vMovieInfo[i].tfile.c_str())? m_vMovieInfo[i].tfile.c_str() : DATADIR "/icons/nopreview.jpg");
